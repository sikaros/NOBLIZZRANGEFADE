...
ns.settings = {
    alpha = 0.40  -- Default: subtle grey overlay
}
...
local function SaveSettings()
    ...
    NoBlizzRangeFadeDB.overlayAlpha = ns.settings.alpha
end

-- Load settings from disk
local function LoadSettings()
    NoBlizzRangeFadeDB = NoBlizzRangeFadeDB or {}
    local storedAlpha = tonumber(NoBlizzRangeFadeDB.overlayAlpha)
    local legacyAlpha = tonumber(NoBlizzRangeFadeDB.alpha)

    -- Old versions used alpha for the real frame. Reuse old custom tint values,
    -- but treat old 1.0 values as unset so the overlay does not become opaque.
    if not storedAlpha and legacyAlpha and legacyAlpha < 1.0 then
        storedAlpha = legacyAlpha
    end

    local a = storedAlpha or 0.40
    -- Clamp to the safe range so a stored value cannot hide the overlay.
...
SlashCmdList["NORANGEFADE"] = function(args)
    ...
    if alphaValue then
        ...
        if value < 0.3 or value > 1.0 then
            print("|cffff0000[NoBlizzRangeFade]|r Overlay alpha must be between 0.3 and 1.0")
            return
        end
        ...
        print("|cff00ff00[NoBlizzRangeFade]|r Overlay alpha set to: " .. string.format("%.2f", value))
        return
    end
    ...
    if args == "test" then
        ...
        print("  Current setting: overlay alpha=" .. string.format("%.2f", ns.settings.alpha))
        for i = 1, 5 do
            pcall(function()
    ...
    elseif args == "status" then
        ...
        print("  Overlay alpha: " .. string.format("%.2f", ns.settings.alpha))
        ...
        print("  /norangefade alpha <0.3-1.0> - Set out-of-range overlay alpha")
        print("  /norangefade test - Check current frame alphas")
        print("  /norangefade debug  - Toggle debug logging")
        print("Example: /norangefade alpha 0.40")
    end
end
